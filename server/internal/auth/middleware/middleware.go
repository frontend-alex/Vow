...
import (
	...
	httpx "vow/server/internal/shared/http"
)
...
// DefaultErrorHandler writes authentication errors using the shared API error format.
func DefaultErrorHandler(w http.ResponseWriter, r *http.Request, err error) {
	if errors.Is(err, ErrForbidden) {
		httpx.Error(w, apperrors.New(apperrors.AuthForbiddenAccess))
		return
	}
	httpx.Error(w, apperrors.New(apperrors.AuthUnauthorized))
}
